    ...
    {   
        ...
        bool IsRagdollSleeping()
        {
            ...
            foreach (Collider c in colls) {
                if (Mathf.Abs(c.attachedRigidbody.linearVelocity.x) <= 0.3f && Mathf.Abs(c.attachedRigidbody.linearVelocity.y) <= 0.3f && Mathf.Abs(c.attachedRigidbody.linearVelocity.z) <= 0.3f) {
                    sleepingRB++;
                }
            }
